...
//   const { blogId } = useParams();
  const { blogId } = params;
  console.log("blodId in blogPage",blogId);
  // let blog = blogId ? useGetBlog(blogId) : null;
  let blog = useGetBlog(blogId);
      ...
      <div className="w-4/5 h-full flex flex-col gap-12 py-12">
        <div className="flex flex-col gap-6 ">
          ...
          <div className="flex gap-4 items-center">
            <img src="https://ps.w.org/user-avatar-reloaded/assets/icon-256x256.png?rev=2540745" className="w-10 h-10" loading="lazy" />
            <div>
              <p className="text-xl text-slate-300">{blog.user.username}</p>
              <p className="text-sm text-slate-400">{blog.publishedTime && new Date(blog.publishedTime).toDateString()}</p>
            </div>
          </div>
          <div className="flex justify-between items-center border-y border-y-slate-700 py-3">
            <div className="flex items-center gap-2 px-2">
              <BiSolidLike
                className="text-slate-400 h-6 w-6 active:text-slate-200 cursor-pointer hover:text-slate-300"
                onClick={() => {
                  if (authUser) {
            ...
            {blog.user.email==authUser && <div
              className="flex gap-1 items-center cursor-pointer pr-8"
              onClick={() => {
        ...
        <div className="flex flex-col gap-8">
          {blog.content.map((para,ind) => {
            return <p key={ind} className="text-slate-300 font-medium font-mono text-[18.5px] tracking-wide leading-normal">{para}</p>;
          })}
        </div>
        <div className="flex flex-wrap gap-4 py-3">
          {blog.topicTags.map((tag,ind) => {
            return (
              <button key={ind}
                className="p-2 px-5 rounded-3xl text-slate-300
                 border border-slate-600 text-base bg-[#252525]"
              >
                {tag}
              </button>
            );
